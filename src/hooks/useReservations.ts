...
export const useReservations = () => {
  ...
  const getAvailableSlots = async (date: string): Promise<AvailableSlot[]> => {
    ...
    return timeSlots.map(time => {
      ...
      const reserved = slotReservations.reduce((sum: number, r: any) => sum + r.number_of_people, 0);
  ...
  const createReservationMutation = useMutation({
    ...
    onSuccess: (data) => {
      ...
      const totalPeople = data?.reduce((sum: number, r: any) => sum + (r?.number_of_people || 0), 0) || 0;
      const totalPrice = data?.reduce((sum: number, r: any) => sum + (r?.total_price || 0), 0) || 0;
  ...
  const { data: tourTypes, isLoading: tourTypesLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      if (error) {
        ...
        return [
          { id: '1', name: 'Passeio panorâmico pela vila', base_price: 10, duration_minutes: 60 },
          { id: '2', name: 'Vila Nova de Milfontes → Praia das Furnas', base_price: 14, duration_minutes: 90 },
          { id: '3', name: 'Travessia pela ponte', base_price: 10, duration_minutes: 45 },
        ];
      }
